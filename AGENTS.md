...
* **HTTP cutover (`#40`):** HTTP server/client/route-group expansion uses decks and `CodegenPlanner`; model artifacts use neutral `TemplateView` / `HttpNeutralModelTemplateAttributes`. Server and client SSP templates still consume legacy `HttpCodegenTemplateView` and `HttpModelTypeNames.referencedModelTypeNames` via the renderer bridge until those templates migrate.
...
* **Codegen cleanup (`#42` in progress):** Remove dead code from the pre-cutover pipeline where templates no longer reference it; keep legacy bridge code until remaining SSP templates migrate. Track remaining bridge surface in [`codegen-core.md`](.ai-doc-reference/codegen-core.md).

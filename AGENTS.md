...
* **Artifact decks are JSON, not Scala (HTTP and SQL):** every bundled output deck is an `outputs.json` resource sitting beside that language's templates (`templates/python/src/http/{server,client,models}/outputs.json`, `templates/python/src/db/outputs.json`, `templates/typescript/src/http/**/outputs.json`), decoded by `planning/config/CodegenOutputDecoders` + `CodegenOutputDeck` in codegen-core. There are **no hardcoded language paths in Scala**: `CodegenOutputDeckLoader.load(templateDirectory, classLoader)` derives the resource path from the (language-encoding) template directory (`<dir>/outputs.json`), and the default template dir comes from `languageId`. A missing deck is a validation error ("missing codegen output deck: …"); a present deck whose referenced `.ssp` templates are absent is a separate "missing required templates" error. Copy-verbatim resources are identified language-neutrally by *not* ending in `.ssp`. A custom `templateDirectory` must ship its own `outputs.json` deck.

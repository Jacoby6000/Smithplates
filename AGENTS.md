...
* **SQL cutover (`#39`):** SQL bundled output expansion uses `CodegenOutput` decks and `CodegenPlanner`; service templates render through `SqlNeutralServiceTemplateAttributes` (`TemplateView` envelope + legacy `ServiceTemplateView` for fragment/includes). Legacy string IR (`SqlShapeIr`, `SqlIrTypeNameResolver`, import helpers) remains for extraction, query rendering, and template data building until fragments migrate to neutral accessors.
...
* **Codegen cleanup (`#42` in progress):** Remove dead code from the pre-cutover pipeline where templates no longer reference it. HTTP legacy bridge removed; SQL planner render path uses `SqlNeutralServiceTemplateAttributes` (neutral `TemplateView` envelope). Fragment partials still consume legacy `ServiceTemplateView` via includes. Track remaining surface in [`codegen-core.md`](.ai-doc-reference/codegen-core.md).

...
* `smithplates-sql-service-ir` — query/service IR (`SqlServiceIr`), extractors, and additive neutral extraction (`SqlCoreModelExtractor` / `SqlMeta` under `sql.service.core` → `ModelSet[SqlMeta]` + `ServiceModel` via `smithplates-smithy-neutral`; legacy string IR still used for query rendering and the SSP bridge)
...
* `smithplates-http-ir` — HTTP trait IDL/classes, `@httpService` extraction, HTTP service IR, projection transforms, and additive neutral extraction (`HttpCoreModelExtractor` → `ModelSet[HttpMeta]` + `ServiceModel` via `smithplates-smithy-neutral`; legacy string IR still used for server/client SSP bridge templates)

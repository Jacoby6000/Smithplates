...
private[query] object SqlQueryRendering {
  ...
  private def renderDeleteQuery(query: SqlDeleteQuery): SqlRenderedQuery = {
    val builder = SqlQuerySegmentBuilder.empty
  ...
  private def renderSelectQuery(query: SqlSelectQuery): SqlRenderedQuery = {
    val projections =
  ...
  private def renderJoinClause(join: com.jacoby6000.smithplates.sql.service.SqlSelectJoin): String = {
    val joinKeyword =
      join.joinType match {

...
operation input: `input: DerivedStruct` (with `use stache.codegen.sql#DerivedStruct`).
...
operation input must be `stache.codegen.sql#DerivedStruct`. Insert columns are every
...
operation input must be `stache.codegen.sql#DerivedStruct`. Codegen expands input to
...
`stache.codegen.sql#DerivedStruct`; codegen expands a result structure from `projections`.

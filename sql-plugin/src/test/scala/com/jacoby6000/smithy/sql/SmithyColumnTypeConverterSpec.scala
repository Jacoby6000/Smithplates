...
final class SmithyColumnTypeConverterSpec extends FunSuite {
  ...
  private def sqlTraitUses(traitLines: List[String]): List[String] = {
    ...
    List(
      Option.when(text.contains("sqlUuid"))("jacoby6000.codegen.sql#sqlUuid"),
      Option.when(text.contains("sqlVarchar"))("jacoby6000.codegen.sql#sqlVarchar"),
      Option.when(text.contains("sqlJson"))("jacoby6000.codegen.sql#sqlJson")
    ).flatten
  }
  ...
  test("Uuid - prelude String with @sqlUuid maps to Uuid") {
    val model = builder.assemble(
      """use jacoby6000.codegen.sql#sqlUuid
        |

...
object PythonCodegenWorkspace {
  ...
  def assertGeneratedIntegrationTests(caseRoot: Path, artifacts: List[SqlCodegenArtifact]): Unit = {
    val testDirectory =
      artifacts
        .collectFirst { case artifact if artifact.kind == SqlServiceCodegenArtifactKind.Test => artifact }
        .map(artifact => caseRoot.resolve(artifact.relativePath).getParent)
        .getOrElse(throw new AssertionError(s"No test artifacts written under $caseRoot"))

    runPytest(testDirectory, pythonImportPath(caseRoot, artifacts))
  }
  ...
  def validateCase(caseName: String, artifacts: List[SqlCodegenArtifact]): Unit = {
    ensureEnvironment()
    ...
    if (artifacts.exists(_.kind == SqlServiceCodegenArtifactKind.Test)) {
      assertGeneratedIntegrationTests(caseRoot, artifacts)
    }
  }
...
"""
  ...
  private def runPytest(testDirectory: Path, importPath: String): Unit =
    runUvCommand(
      Seq("run", "pytest", testDirectory.toString),
      environment = Seq("PYTHONPATH" -> importPath)
    ).fold(
      message => throw new AssertionError(s"pytest failed:\n$message"),
      _ => ()
    )

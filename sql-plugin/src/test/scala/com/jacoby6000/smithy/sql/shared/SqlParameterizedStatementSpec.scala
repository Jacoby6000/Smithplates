...
final class SqlParameterizedStatementSpec extends FunSuite {
  ...
  test("inferForCodegen - maps dialects to bundled driver placeholders") {
    assertEquals(
      SqlBindPlaceholder.inferForCodegen(com.jacoby6000.smithy.sql.SqliteDialect),
      SqlBindPlaceholder("?")
    )
    assertEquals(
      SqlBindPlaceholder.inferForCodegen(com.jacoby6000.smithy.sql.PostgresDialect),

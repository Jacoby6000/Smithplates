...
class SqlQueryExtractorSpec extends munit.FunSuite {
  ...
  test("DeriveInsert - fails when input is Unit") {
    ...
    assert(
      result.swap.toOption.get.exists {
        case InvalidDeriveInsert(_, reason) if reason.contains("jacoby6000.codegen.sql#DerivedStruct") => true
        case _                                                                                            => false
      }
    )
  }
  ...
  test("DeriveUpdate - fails when input is not DerivedStruct") {
    ...
    assert(
      result.swap.toOption.get.exists {
        case InvalidDeriveUpdate(_, reason) if reason.contains("jacoby6000.codegen.sql#DerivedStruct") => true
        case _                                                                                            => false
      }
    )
  }
  ...
  test("DeriveDelete - fails when input is not DerivedStruct") {
    ...
    assert(
      result.swap.toOption.get.exists {
        case InvalidDeriveDelete(_, reason) if reason.contains("jacoby6000.codegen.sql#DerivedStruct") => true
        case _                                                                                            => false
      }
    )
  }
  ...
  test("DeriveSelectOne - fails when input is not DerivedStruct") {
    ...
    assert(
      result.swap.toOption.get.exists {
        case InvalidDeriveSelectOne(_, reason) if reason.contains("jacoby6000.codegen.sql#DerivedStruct") => true
        case _                                                                                              => false
      }
    )
  }
  ...
  test("Update - requires primary key members and rejects database-managed members") {
    val model = SqlTestModelBuilder.assemble(
      """
        ...
        |use jacoby6000.codegen.sql#sqlPrimaryKey
        |use jacoby6000.codegen.sql#sqlTable
        |use jacoby6000.codegen.sql#sqlUpdate
        |use jacoby6000.codegen.sql#sqlUpdatedTimestamp
        |
        |@sqlTable(name: "widgets")
        |structure Widget {

...
import com.jacoby6000.smithy.mustachetest.{
  MustacheTemplateLanguageBackend,
  MustacheTemplateTestCase,
  MustacheTemplateVariant
}
import com.jacoby6000.smithy.sql.{PostgresDialect, SqlDialect, SqlModelExtractor, SqlTestModelLoader, SqliteDialect}
    ...
    new SqlServiceCodegenTestBackend {
      ...
      def render(testCase: MustacheTemplateTestCase, model: Model): Either[String, Map[String, String]] = {
        ...
        SqlModelExtractor
          .extract(model)
          .andThen { schema =>
            SqlServiceCodegenRenderer.render(model, schema, settingsForTests).map { renderedArtifacts =>

...
object SqlQueryRenderer {
  ...
  private def appendComparisonClause(
      ...
      predicates: List[com.jacoby6000.smithy.sql.SqlSelectPredicate],
      keyword: String,
      prefix: String = "\n"
  ): Unit =
  ...
  private def appendPredicateOperand(
      builder: SqlQuerySegmentBuilder,
      operand: com.jacoby6000.smithy.sql.SqlPredicateOperand
  ): Unit =
    operand match {
      case com.jacoby6000.smithy.sql.SqlPredicateOperand.InputMember(_) =>
        builder.appendParameter()
      case com.jacoby6000.smithy.sql.SqlPredicateOperand.TableColumn(column) =>
        builder.appendText(s"${column.tableAlias}.${column.columnName}")
      case com.jacoby6000.smithy.sql.SqlPredicateOperand.Projection(projection) =>
  ...
  private def renderJoinClause(join: com.jacoby6000.smithy.sql.SqlSelectJoin): String = {
    val joinKeyword =
      join.joinType match {

...
import com.jacoby6000.smithy.sql.SqlDeriveSelectReferenceResolver.{ResolvedReference, SelectScope}
import com.jacoby6000.smithy.sql.SqlSelectTableContext.TableContext
import com.jacoby6000.smithy.sql.traits.{
...
private[sql] object SqlDeriveSelectExtractor {
  ...
  def extractDeriveSelects(model: Model, schema: SqlSchema): SqlValidated[List[SqlSelectQuery]] =
    model.getOperationShapes.asScala.toList
      .flatMap { operation =>
        SmithySqlTraitAccess.sqlDeriveSelect(operation).map(selectTrait => (operation, selectTrait))
      }
      .traverse { case (operation, selectTrait) =>
        extractDeriveSelect(model, schema, operation, selectTrait)
      }

  private def extractDeriveSelect(
      model: Model,
      schema: SqlSchema,
  ...
  private def requireInputStructure(
      model: Model,
      operation: OperationShape
  ...
  private def validateOptionalInputMember(
  ...
  private def validateStarProjectionsCompatibility(
      operationShape: ShapeId,
  ...
  private def extractProjections(
      operationShape: ShapeId,
      projections: SqlDeriveSelectProjectionsValue,
  ...
  private def resolveConditionOperand(
      operationShape: ShapeId,
      reference: String,
      scope: SelectScope,
      clause: FilterClause,
  ...
  private def validateConditionOperands(
      operationShape: ShapeId,
  ...
  private def validateGroupByCoversSelectColumns(
      operationShape: ShapeId,
      selectColumns: List[SqlSelectProjection],
  ...
  private def clauseName(clause: FilterClause): String =
    clause match {
      case FilterClause.Where  => "where"
      case FilterClause.Having => "having"
    }

  private def invalidCondition(
      operationShape: ShapeId,
      reference: String,
      clause: FilterClause,
      reason: String

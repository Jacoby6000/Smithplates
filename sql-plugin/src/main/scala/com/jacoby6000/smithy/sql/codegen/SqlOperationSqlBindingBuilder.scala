...
import com.jacoby6000.smithy.sql.shared.{SqlParameterizedStatement, SqlQueryRenderer}
...
object SqlOperationSqlBindingBuilder {
  ...
  private def buildInsertBinding(
      model: Model,
      operation: SqlOperation,
      query: com.jacoby6000.smithy.sql.SqlInsertQuery,
  ...
  private def buildUpdateBinding(
      model: Model,
      query: com.jacoby6000.smithy.sql.SqlUpdateQuery,
      statement: SqlParameterizedStatement
  ): SqlCodegenSqlBinding =
    SqlCodegenSqlBinding(
      queryKind = "update",
  ...
  private def buildDeleteBinding(
      model: Model,
      query: com.jacoby6000.smithy.sql.SqlDeleteQuery,
      statement: SqlParameterizedStatement
  ): SqlCodegenSqlBinding =
    SqlCodegenSqlBinding(
      queryKind = "delete",
  ...
  private def buildSelectOneBinding(
      model: Model,
      operation: SqlOperation,
      query: com.jacoby6000.smithy.sql.SqlSelectOneQuery,
      statement: SqlParameterizedStatement
  ): SqlCodegenSqlBinding = {
    val resultFields =
      ...
      tableStructure.getAllMembers.asScala.toList
        .collectFirst {
          case (memberName, member)
              if com.jacoby6000.smithy.sql.SmithySqlTraitAccess.columnName(memberName, member) == columnName =>
            val resolved = SqlCodegenTypeResolver.resolveMember(model, memberName, member)
            jsonResultField(
              member,
              memberName,

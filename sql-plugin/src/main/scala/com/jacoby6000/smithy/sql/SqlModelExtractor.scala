...
object SqlModelExtractor {
    ...
    model.getStructureShapes.asScala.toList
      ...
      .andThen { schema =>
        (
          ...
          schema.copy(queries = queries, services = services)
        }
      }
  ...
  private def extractTable(structure: StructureShape, model: Model): SqlValidated[SqlTable] = {
    val members = com.jacoby6000.smithy.sql.shared.SqlTableMemberOrdering.orderedMembers(structure)

    com.jacoby6000.smithy.sql.shared.SqlTableMemberOrdering.validate(structure).andThen { _ =>

...
import com.jacoby6000.smithy.sql.it.SqlDdlSupport
...
object SqliteContainerSupport {
  ...
  def applyDdl(container: GenericContainer, ddl: String): Unit = {
    container.container.execInContainer("mkdir", "-p", dbDirectory)
    SqlDdlSupport.splitStatements(ddl).foreach { statement =>
      val exitCode = runSqlScript(container, statement)
      if (exitCode != 0) {

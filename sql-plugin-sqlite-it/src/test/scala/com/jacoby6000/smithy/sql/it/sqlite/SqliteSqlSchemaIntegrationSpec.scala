package com.jacoby6000.smithy.sql.it.sqlite
...
import com.jacoby6000.smithy.sql.sqlite.SqliteRenderer
...
final class SqliteSqlSchemaIntegrationSpec extends FunSuite with TestContainerForEach {
  ...
  test("simple schema applies and enforces foreign keys") {
    withContainers { sqlite =>
      val ddl = SqlDdlSupport.renderDdl(SqliteRenderer, SqlIntegrationSchemas.simpleSchema)
      SqliteContainerSupport.applyDdl(sqlite, ddl)
  ...
  test("complex schema creates all tables and foreign keys") {
    withContainers { sqlite =>
      val ddl = SqlDdlSupport.renderDdl(SqliteRenderer, SqlIntegrationSchemas.complexSchema)
      SqliteContainerSupport.applyDdl(sqlite, ddl)
  ...
  test("varchar CHECK rejects values longer than maxLength") {
    withContainers { sqlite =>
      val ddl = SqlDdlSupport.renderDdl(SqliteRenderer, SqlIntegrationSchemas.varcharCheckSchema)
      SqliteContainerSupport.applyDdl(sqlite, ddl)

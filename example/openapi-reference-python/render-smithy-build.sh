#!/usr/bin/env bash
# Render smithy-build.json from template using the current Smithplates build version.
set -euo pipefail
...
envsubst '${SMITHPLATES_VERSION} ${SMITHY_VERSION}' \
  < "${example_root}/smithy-build.json.template" \
  > "${example_root}/smithy-build.json"
echo "rendered ${example_root}/smithy-build.json (smithplates ${SMITHPLATES_VERSION})"

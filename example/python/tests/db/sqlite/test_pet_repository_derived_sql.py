...
async def pet_repository_service() -> AsyncIterator[PetRepositoryAiosqliteService]:
    ...
    try:
        yield PetRepositoryAiosqliteService(connection)
    finally:
...
@pytest.mark.sqlite
@pytest.mark.asyncio
async def test_derived_sql_methods_transaction_commit(pet_repository_service: PetRepositoryAiosqliteService) -> None:
    ...
    try:
        ...
        fetched = await pet_repository_service.get_pet_record(id=entity_id, transaction=connection)
        assert isinstance(fetched, GetPetRecordResult)
        assert fetched.name == "integration-name"
        assert fetched.status == "available"
        assert fetched.species == 3
